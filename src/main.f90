program main
    ...
    double precision::epot, ekin, temperature, deltag, ekin_paralel
    ...
    do tt = 1, ntimes, 1
        ...
        elseif (thermo .eq. 1) then
            call vel_verlet_with_thermo(natoms, r, v, F, epot, dt, rc, L, temp, &
                                        pressp, gr, deltag, particle_range, &
                                        interact_range, interact_list, sizes, &
                                        displs)
        else
		...
		!print*, "ekin before:",ekin_paralel
		if (taskid.eq.0) then
        	temperature = 2.d0*ekin/(3.d0*dble(natoms) - 3.d0)
			!print*, "temp:",temperature, "ekin:",ekin_paralel
			ngr=ngr+1
		endif
    ...
    if (taskid .eq. 0) then
        ...
        close (15); close (12); close (13); close (14)
        print *, "after rdf"
    end if

    !call MPI_BARRIER(MPI_COMM_WORLD, ierror)
    !deallocate (r, v, F)
    !deallocate (F_root, gr, interact_list)
